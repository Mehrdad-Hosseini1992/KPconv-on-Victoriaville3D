...
class NPM3DDataset(PointCloudDataset):
    ...
    def __init__(self, config, set='training', use_potentials=True, load_data=True):
        ...
        self.label_to_names =  {0: 'Other',      # Trash-can, Bench, Bicycle-Parking, Pedestrian, Box, Pot, Fire Hydrant, Barrier, curb of jardine, Ad-Board, Wire, Car
                                1: 'Sidewalk', 
                                2: 'Road', 
                                3: 'Curb-cut', 
                                4: 'Vegetation', 
                                5: 'Tree',      
                                6: 'Post',       # Pole-Lamp post, Traffic-pole 
                                7: 'Ramp',      
                                8: 'Road sign', # Road sign, Bollard, Snow-Stick                                
                                9: 'Building',  # Building, Fans, Stair Rod
                                10: 'Step' ,
                                11: 'Door'
                                }
        ...
        self.ignored_labels = np.array([], dtype=np.int64)

        # Dataset folder
        self.path = '/home/mehrdad/datasets/Victoriaville/2iem-exame/'

        # Add and initialize the val_confs attribute
        self.val_confs = []
        all_class_ious = []
        ...
        self.train_path = 'Mehrdad-Victoriaville'
        self.original_ply_path = 'Test'

        # List of files to process
        ply_path = join(self.path, self.train_path)

        # Proportion of validation scenes
        self.cloud_names = ['ND-Train-1', 'ND-Train-2', 'ND-Train-3', 'ND-Train-4', 'Validation', '01']
        self.all_splits = [0, 1, 2, 3, 4, 5]
        self.validation_split = 4
        self.test_cloud_names = ['01']
        self.test_splits = [5]
        self.train_splits = [0, 1, 2, 3]
    ...
    def _label_to_idx(self, label):
        if label in self.ignored_labels:
            ...
            converted_label = int(class_mapping.get(label, 0))
            return converted_label
            
    def convert_labels(self, labels):
        """Converts original labels to new class representation."""
        converted_labels = np.zeros_like(labels)
        for i, label in enumerate(labels):
            converted_labels[i] = self.label_to_idx(label)
        return converted_labels.astype('int64') # Ensure the data type is correct
    ...
    def random_item(self, batch_i):
        ...
        while True:
            ...
            if self.set in ['test', 'ERF']:
                ...
                input_labels = self.input_labels[cloud_ind][input_inds]
                # print("Original labels:", np.unique(input_labels, return_counts=True)) 
                input_labels = self.convert_labels(input_labels)
                # print("Converted labels:", np.unique(input_labels, return_counts=True))
    ...
    def prepare_NPM3D_ply(self):
        ...
        for cloud_name in self.cloud_names:
            ...
            # Initiate containers
            cloud_x = original_ply['x'].astype(np.float32)
            cloud_y = original_ply['y'].astype(np.float32)
            cloud_z = original_ply['z'].astype(np.float32)
            ...
            if cloud_name in ['01']:
                ...
                labels = original_ply['scalar_class']
                labels = labels.astype(np.int32)
                labels = labels.reshape(len(labels), 1)

                # Save as ply
                field_names = ['x', 'y', 'z', 'scalar_class']
    ...
    def load_subsampled_clouds(self):
        ...
        for i, file_path in enumerate(self.files):
            ...
            if exists(KDTree_file):
                ...
                #------------------------------------------------------------------------------------#
                ######Find the shortest coordinates (New code)
                shortest_x = np.min(points[:, 0])
                shortest_y = np.min(points[:, 1])
                shortest_z = np.min(points[:, 2])

                ######### Shift the point cloud (New code)
                points[:, 0] -= shortest_x
                points[:, 1] -= shortest_y
                points[:, 2] -= shortest_z
                ...
                ########### Append the shifted point cloud to the list for global normalization
                ...
                if self.set == 'test':
                    ...
                    print("Original labels before conversion:", np.unique(labels, return_counts=True))
                    labels = self.convert_labels(labels)
                    print(f"Unique labels after mapping in {cloud_name}: {np.unique(labels)}")
                ...
                # *************************************************************************
                # Modified Subsampling with Point Count Threshold 
                # *************************************************************************

                threshold = 1000000  # Threshold for applying grid subsampling

                # Subsample, applying grid subsampling only to classes above the threshold
                sub_points = []
                sub_labels = []
                for label in np.unique(labels):
                    ...
                    if len(class_points) > threshold:
                        # Apply grid subsampling to classes with more than threshold points
                        class_sub_points, class_sub_labels = grid_subsampling(class_points,
                                                                            labels=class_labels,
                                                                            sampleDl=dl)
                        # Ensure class_sub_labels is 2-dimensional
                        class_sub_labels = class_sub_labels.reshape(-1, 1)  

                        sub_points.append(class_sub_points)
                        sub_labels.append(class_sub_labels)
                    else:
                        # Keep all points for classes with fewer than threshold points
                        sub_points.append(class_points)
                        sub_labels.append(class_labels.reshape(-1, 1))  # Ensure 2D shape


                sub_points = np.concatenate(sub_points)
                sub_labels = np.concatenate(sub_labels)

                # *************************************************************************

                print(f"Unique labels after subsampling in {cloud_name}: {np.unique(sub_labels, return_counts=True)}")
                ...
                # Calculate overall minimum and maximum coordinates across all point clouds
                overall_min_coords = np.min(np.concatenate([pc.min(axis=0) for pc in all_point_clouds]), axis=0)
                overall_max_coords = np.max(np.concatenate([pc.max(axis=0) for pc in all_point_clouds]), axis=0)

                # Normalize each point cloud to the [0, 1] range
                for i in range(len(all_point_clouds)):
                    all_point_clouds[i] -= overall_min_coords
                    all_point_clouds[i] /= (overall_max_coords - overall_min_coords)
                # *************************************************************************


                # Get chosen neighborhoods
                # search_tree = KDTree(sub_points, leaf_size=10)
                search_tree = [KDTree(pc, leaf_size=10) for pc in all_point_clouds]
            ...
            for i, file_path in enumerate(self.files):
                ...
                if exists(proj_file):
                    ...
                    if self.set == 'test':
                        ...
                        labels = data['scalar_class'].astype(int) 
    ...
    def load_evaluation_points(self, file_path):
        ...
        if self.set == 'validation':
            data['scalar_class'] = self.convert_labels(data['scalar_class']) # Convert the labels if it's a validation file.
...
class NPM3DCustomBatch:
    ...
    def __init__(self, input_list):
        ...
        # Print unique labels after batch creation
        # print("Labels after batch creation:", np.unique(self.labels.cpu().numpy(), return_counts=True))
        return
