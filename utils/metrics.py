...
def fast_confusion(true, pred, label_values=None):
    ...
    if label_values is None:
      ...
      # Check if label_values is a NumPy array and of int64 dtype
        if not isinstance(label_values, np.ndarray) or label_values.dtype != np.int64:
            # If not, convert it to an int64 array
            label_values = np.array(label_values, dtype=np.int64)

        # Check if label values are unique
    ...
    if num_classes == 0: #Add this section
      # Handle case where there are no valid labels
      return np.zeros((len(np.unique(true)), len(np.unique(true))), dtype=np.int32)
    # Start confusion computations
    elif label_values[0] == 0 and label_values[-1] == num_classes - 1:
