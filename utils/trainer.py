...
class ModelTrainer:
    ...
    def __init__(self, net, config, chkp_path=None, finetune=False, on_gpu=True):
        ...
        if (chkp_path is not None):
            if finetune:
                # Instead of directly loading, call your custom function
                net.load_pretrained_weights(chkp_path) 
                net.train()
    ...
    def train(self, net, training_loader, val_loader, config):
        ...
        # ######## Initialize best_miou and best_model_wts (New)
        # best_miou = 0.0
        ...
        for epoch in range(config.max_epoch):
            ...
            train_losses = []
            train_accuracies = []
            val_mious = []  # Initialize val_mious
            ...
            if val_loader is not None:
                ...
                net.train()
                val_mious.append(mIoU)  # Accumulate validation mIoU
        ...
        ###Final Plotting after training
        # self.plot_loss_accuracy(net, config)
        self.plot_loss_accuracy(net, config, train_losses, train_accuracies, val_mious, self.moving_average)  # Pass all_mious
    ...
    def plot_loss_accuracy(self, net, config, train_losses, train_accuracies, val_mious, moving_average):   
        """
        Function to plot the training loss, accuracy, and validation mIoU curves.
        """

        # Read the training log (this part remains the same)
        log_file_path = join(config.saving_path, 'training.txt')
        ...
        # Process log file (this part remains the same)
        for line in lines:
            epoch, step, out_loss, offset_loss, train_accuracy, _ = line.split()
            steps.append(int(step))
            total_losses.append(float(out_loss) + float(offset_loss))
            out_losses.append(float(out_loss))
            offset_losses.append(float(offset_loss))
        ...
        total_losses_smooth = moving_average(total_losses, window_size)
        train_accuracies_smooth = moving_average(train_accuracies, window_size)
        smoothed_steps = steps[:len(total_losses_smooth)]  # Adjust steps after smoothing
        ...
        # Loss Curve
        plt.subplot(2, 2, 1) 
        plt.plot(smoothed_steps, total_losses_smooth, label='Total Loss (Smoothed)')
        ...
        # Accuracy Curve
        plt.subplot(2, 2, 2) 
        plt.plot(smoothed_steps, train_accuracies_smooth, label='Accuracy (Smoothed)')
        ...
        # mIoU Curve 
        plt.subplot(2, 1, 2) 
        if val_mious:  # Check if val_mious contains values
            plt.plot(val_mious)  
            ...
            print("No validation mIoU data available.")
            plt.title('No mIoU data')
